...
const StudentCoursesCalendar: React.FC<StudentCoursesCalendarProps> = ({ courses }) => {
    ...
    const handleWeekChange = (direction: 'next' | 'previous') => {
        ...
        const newWeek = getWeekDates(new Date(referenceDate.setDate(referenceDate.getDate() + offset)));
        ...
        <div className="overflow-x-auto">
            ...
            <table className="table-auto w-full border border-gray-300">
                <thead>
                    <tr className="bg-ternary-color text-white">
                        {currentWeek.map((date) => {
                            ...
                            return (
                                <th key={date.toISOString()} className="px-4 py-2 text-center bg-[#E0F7F9]">
                ...
                <tbody>
                    <tr>
                        {currentWeek.map((date) => (
                            <td key={date.toISOString()} className="px-4 py-2 align-top border border-gray-300">
                                <div className="flex flex-col flex-wrap gap-4 justify-center">
                                    {coursesByDate[date.toLocaleDateString('fr-CA')]?.map((course) => (
